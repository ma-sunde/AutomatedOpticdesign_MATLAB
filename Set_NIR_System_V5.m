...
% Material Catalog für 'PMMA' hinzufügen
...
if TheSystem.SystemData.Fields.NumberOfFields == 1
    ...
    mf_filename = System.String.Concat(FolderVIS, 'MF\Startsystem_NIR.MF');
...
%----------- Linsen 3 und 4 des VIS-Systems einfügen --------------------------
% Die neuen Linsen werden zuerst eingfügt, bevor die Platzhalter gelöscht
% werden. Wenn die Platzhalter zuerst gelöscht werden, gehen evtl.
% Verknüpfungen zu den Oberflächen aus der MF verloren, weil der Bezug zur
% Oberflächennummer fehlt. 
...
%----------- Platzhalterlinsen 3 und 4 löschen --------------------------

 for i=1:4
     TheLDE.DeleteRowAt(9);
 end   
...
if TheSystem.SystemData.Fields.NumberOfFields > 1
    ...
    for i=2:(size(GENF_Values,1))           %Matrix Systeme bekommen den CENY Operanden aus der Tabelle der GENF Zielwerte, weil ursprünglich
                                            %nur Punktlichtquellen
                                            %für das NIR-System vorgesehen
                                            %waren. Alle NIR
                                            %Lichtverteilungen und Zielwerte werden in
                                            %der GENF Tabelle
                                            %abgespeichert.
        TheMFE.InsertNewOperandAt(n);
        Operand = TheMFE.GetOperandAt(n); 
        Operand.ChangeType(ZOSAPI.Editors.MFE.MeritOperandType.CENY);
        Operand.GetCellAt(2).IntegerValue = TheLDE.NumberOfRows-1;      %Hier wird davon ausgegangen, dass die letzte Surface die Image Surface ist
        Operand.GetCellAt(4).IntegerValue = GENF_Values{i,1};
        Operand.Target = GENF_Values{i,4};
        Operand.Weight = 5;
        n = n+1;
