...
%--------------------- Lichtveretilung einlesen --------------------------
Lichtverteilung_Differenz; %entsteht aus der Differenz_Lichtverteilung.m
%size = size(Lichtverteilung);
ResolutionVertikal = abs(Lichtverteilung_Differenz(2,1)-Lichtverteilung_Differenz(3,1));
ResolutionHorizontal = abs(Lichtverteilung_Differenz(1,2)-Lichtverteilung_Differenz(1,3));

% hellsten Punkt finden
[Maximum, MaxPos] = max(Lichtverteilung_Differenz(:));

% Position der hellsten Punkte finden
[rowsOfMaxes, colsOfMaxes] = find(Lichtverteilung_Differenz == Maximum);

% 0° Vertikal 0° Horizontal lokalisieren // vorausgesetzt erste Zeile und
% Spalte beinhalten die Achsenbeschriftung
colofZero = find(Lichtverteilung_Differenz(1,:) == 0);
rowofZero = find(Lichtverteilung_Differenz(:,1) == 0);
...
%am Ende der Tabelle auf. Da der letzte Bereich aus einer Zelle besteht,
...
GENF_Values = cell(NumberOfFields,4);

GENF_Values{1,1} = 'Fieldnummer';
GENF_Values{1,2} = 'Vertikal in °';
GENF_Values{1,3} = 'Horizontal in °';
GENF_Values{1,4} = 'Y-Position in mm';

[Zeilen,Spalten] = size(GENF_Values);

    for i=1:Zeilen
        GENF_Values{i+1,1} = i;
        GENF_Values{i+1,2} = Schwerpunkte(i,4); 
        GENF_Values{i+1,3} = Schwerpunkte(i,5);
        GENF_Values{i+1,4} = abs(tand(Schwerpunkte(i,5))*25*10^03);     %Es wird der Betrag genommen, da die Lichtverteilung vom Mittelpunkt bis zur
    end                                                                 %Ecke oben links abgetastet wurde. In dem Bereich sind die Horizontal-Koordinaten negativ
                    %Muss evtl. noch umgerechnet werden. Hier wurde
                    %schlicht mit 25 Metern Entfernung gerechnet. 
